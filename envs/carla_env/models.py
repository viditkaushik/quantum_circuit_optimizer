...
class CarlaAction(Action):
    ...
    lane_direction: str | None = Field(default=None, description="Lane change direction (deprecated, use target_lane_id)")
...
class CarlaObservation(Observation):
    ...
    goal_distance: float | None = Field(default=None, description="Distance to goal in meters (if goal is set)")
    goal_direction: str | None = Field(default=None, description="Direction to goal: forward, left, right, or behind")
    ...
    collided_with: str | None = Field(default=None, description="ID of actor collided with")
    ...
    rubric_reward: float | None = Field(default=None, description="Reward computed by the rubric for RL training")

    # Camera capture (only populated when capture_image action is used)
    camera_image: str | None = Field(default=None, description="Base64-encoded JPEG image from front-facing camera")
